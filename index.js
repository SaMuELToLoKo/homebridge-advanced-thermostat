			...
			return request.get({
	        	...
	        	json = JSON.parse(body);
					if (json.units == 0){
						...
						this.log('Current Temperature is %s (%s)', json.temperature, json.units);
						this.log('The system is set up in Fahrenheit to %s', json.temperature);
	...
	getTargetTemperature: function(callback) {
			...
			return request.get({
				...
				if (json.units == 0) {
					...
					this.log('Target Temperature is %s (%s)', json.tarTemperature, json.units);
					this.log('The system is set up in Fahrenheit to %s', json.tarTemperature);
	...
	setTargetTemperature: function(value, callback) {
		  this.log('Setting Target Temperature from: ', this.apiAdress + '/targetTemperature/' + value);
			if (this.TemperatureDisplayUnits == Characteristic.TemperatureDisplayUnits.FAHRENHEIT) {
				...
				this.log('The system is set up in Fahrenheit to %s', value);
				}
			return request.post({
			...
			return request.get({
				...
				if (!err && response.statusCode === 200) {
					...
					if (json.units == 0) {
						...
						this.log('Target Heat threshoold is %s (%s)', json.heatThreshold, json.units);
						this.log('The system is set up in Fahrenheit to %s', json.heatThreshold);
