...
Thermostat.prototype = {
	...
	getCurrentTemperature: function(callback) {
			...
			return request.get({
	        	...
	        	json = JSON.parse(body);
					if (json.units == 0){
						this.log('Current Temperature in ℃ is %s (%s)', json.temperature, json.units);
	...
	getTargetTemperature: function(callback) {
			...
			return request.get({
				...
				json = JSON.parse(body);
				if (json.units == 0) {
					this.log('Target Temperature in ℃ is %s (%s)', json.tarTemperature, json.units);
					this.TargetTemperature = parseFloat(json.tarTemperature);
				}
				else if (json.units == 1) {
					this.log('Target Temperature in ℉ is %s (%s)', json.tarTemperature, json.units);
					this.TargetTemperature = this.fToC(parseFloat(json.tarTemperature));
				}
	...
	getHeatingThresholdTemperature: function(callback) {
			...
			return request.get({
				...
				if (!err && response.statusCode === 200) {
					...
					json = JSON.parse(body);
					if (json.units == 0) {
						this.log('Target Heat threshoold in ℃ is %s (%s)', json.heatThreshold, json.units);
						this.HeatingThresholdTemperature = parseFloat(json.heatThreshold);
					}
					else if (json.units == 1) {
						this.log('Target Heat threshoold in ℉ is %s (%s)', json.heatThreshold, json.units);
						this.HeatingThresholdTemperature = this.fToC(parseFloat(json.heatThreshold));
					}
	...
	getCoolingThresholdTemperature: function(callback) {
			...
			return request.get({
				...
				if (!err && response.statusCode === 200) {
					...
					json = JSON.parse(body);
					if (json.units == 0) {
						this.log('Target Cool Threshold in ℃ is %s (%s)', json.coolThreshold, json.units);
	...
	getCurrentRelativeHumidity: function(callback) {
	 		this.log('Getting Current Relative Humidity from:', this.apiAdress + '/info');
			return request.get({
				url: this.apiAdress + '/info',
				auth: {
					user: this.username,
